...
macro_rules! safe_deserialize {
    ...
    ( ($name:tt, $key:tt) ) => {
        impl $name {
            pub fn safe_deserialize(data: &[u8]) -> Result<Self, Error> {
...
impl Metadata {
    pub fn safe_deserialize(data: &[u8]) -> Result<Self, Error> {
        if data.is_empty() || data[0] != Key::MetadataV1 as u8 {
            return Err(Error::new(ErrorKind::Other, "DataTypeMismatch"));
        }
        // mutable "pointer" to the account data
        let mut data = data;
    ...
    fn deserialize_unchecked(buf: &mut &[u8]) -> Result<Self, Error> {
        ...
        // V1.2
        let token_standard_res: Result<Option<TokenStandard>, Error> =
            BorshDeserialize::deserialize(buf);
        let collection_res: Result<Option<Collection>, Error> = BorshDeserialize::deserialize(buf);
        let uses_res: Result<Option<Uses>, Error> = BorshDeserialize::deserialize(buf);

        // V1.3
        let collection_details_res: Result<Option<CollectionDetails>, Error> =
            BorshDeserialize::deserialize(buf);

        // pNFT - Programmable Config
        let programmable_config_res: Result<Option<ProgrammableConfig>, Error> =
            BorshDeserialize::deserialize(buf);
