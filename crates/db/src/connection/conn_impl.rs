    ...
    async fn insert_deployment_impl(&self, d: &FlowDeployment) -> crate::Result<DeploymentId> {
        ...
        tx.do_execute(
            ...
            &[
                ...
                &d.action_config.as_ref().map(Json),
                &d.fees.iter().map(Json).collect::<Vec<_>>(),
            ],
        )
        .await
    ...
    async fn get_deployment_impl(&self, id: &DeploymentId) -> crate::Result<FlowDeployment> {
        ...
        let d = FlowDeployment {
            ...
            action_identity: r
                ...
                .transpose()?,
            action_config: r
                .try_get::<_, Option<Json<SolanaActionConfig>>>("action_config")
                .map_err(Error::data("flow_deployments.action_config"))?
                .map(|Json(x)| x),
            fees: r
                .try_get::<_, Option<Vec<Json<(String, u64)>>>>("fees")
                .map_err(Error::data("flow_deployments.fees"))?
                .map(|v| {
                    v.into_iter()
                        .map(|Json((pubkey, amount))| {
                            Pubkey::from_str(&pubkey).map(|pk| (pk, amount))
                        })
