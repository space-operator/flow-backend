...
const NAME: &str = "take_order";
const DEFINITION: &str = flow_lib::node_definition!("limo/take_order.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let taker_input_ata = derive_ata(
        &input.taker.pubkey(),
        &input.input_mint,
        &input.input_token_program,
    );
    let taker_output_ata = derive_ata(
        &input.taker.pubkey(),
        &input.output_mint,
        &input.output_token_program,
    );
