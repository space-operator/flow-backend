...
const NAME: &str = "revoke_direct_recipient";
const DEFINITION: &str = flow_lib::node_definition!("rewards/revoke_direct_recipient.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let (recipient_account, _) = pda::find_direct_recipient(&input.distribution, &input.recipient);
    let (distribution_vault, _) =
        pda::find_ata(&input.distribution, &input.mint, &input.token_program);
    let recipient_token_account =
        pda::find_ata(&input.recipient, &input.mint, &input.token_program).0;
    ...
    let instruction = build_rewards_instruction(
        RewardsDiscriminator::RevokeDirectRecipient,
        accounts,
        args_data,
    );
