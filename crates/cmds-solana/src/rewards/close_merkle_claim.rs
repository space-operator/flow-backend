...
const NAME: &str = "close_merkle_claim";
const DEFINITION: &str = flow_lib::node_definition!("rewards/close_merkle_claim.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let (claim_account, _) = pda::find_merkle_claim(&input.distribution, &input.claimant.pubkey());
    let (event_authority, _) = pda::find_event_authority();

    let accounts = vec![
        AccountMeta::new(input.claimant.pubkey(), true),
    ...
    let instruction =
        build_rewards_instruction(RewardsDiscriminator::CloseMerkleClaim, accounts, vec![]);

    let ins = Instructions {
        lookup_tables: None,
        fee_payer: input.claimant.pubkey(),
        signers: [input.claimant].into_iter().collect(),
