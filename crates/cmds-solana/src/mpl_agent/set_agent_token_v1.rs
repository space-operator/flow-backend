...
use mpl_agent_identity::instructions::SetAgentTokenV1Builder;

const NAME: &str = "set_agent_token_v1";
const DEFINITION: &str = flow_lib::node_definition!("mpl_agent/set_agent_token_v1.jsonc");
...
pub async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let instruction = SetAgentTokenV1Builder::new()
        .agent_identity(to_pubkey_v2(&agent_identity))
        .asset(to_pubkey_v2(&input.asset))
        .genesis_account(to_pubkey_v2(&input.genesis_account))
