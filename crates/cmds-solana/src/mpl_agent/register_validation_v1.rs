...
use super::{find_agent_validation_pda, to_instruction_v3, to_pubkey_v2};
use mpl_agent_validation::instructions::RegisterValidationV1Builder;

const NAME: &str = "register_validation_v1";
const DEFINITION: &str = flow_lib::node_definition!("mpl_agent/register_validation_v1.jsonc");
...
pub async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let (agent_validation, _) = find_agent_validation_pda(&input.asset);

    let mut builder = RegisterValidationV1Builder::new();
    builder
        .agent_validation(to_pubkey_v2(&agent_validation))
