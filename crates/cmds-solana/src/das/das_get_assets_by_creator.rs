//! DAS Get Assets By Creator - Fetch assets by creator address
//!
//! Uses the Metaplex DAS API `getAssetsByCreator` method.

use crate::prelude::*;
use reqwest::header::CONTENT_TYPE;
use serde_json::json;

pub const NAME: &str = "das_get_assets_by_creator";
const DEFINITION: &str = flow_lib::node_definition!("das/das_get_assets_by_creator.jsonc");
...
pub struct Input {
    ...
    pub creator: Pubkey,
    #[serde(default = "default_only_verified")]
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let params = json!({
        "creatorAddress": input.creator.to_string(),
        "onlyVerified": input.only_verified,
        "page": input.page,
        "limit": input.limit
    });
    let cache_key = super::cache::key("getAssetsByCreator", &params);
