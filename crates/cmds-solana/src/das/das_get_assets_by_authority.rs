//! DAS Get Assets By Authority - Fetch assets by update authority
//!
//! Uses the Metaplex DAS API `getAssetsByAuthority` method.

use crate::prelude::*;
use reqwest::header::CONTENT_TYPE;
use serde_json::json;

pub const NAME: &str = "das_get_assets_by_authority";
const DEFINITION: &str = flow_lib::node_definition!("das/das_get_assets_by_authority.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let params = json!({
        "authorityAddress": input.authority.to_string(),
        "page": input.page,
        "limit": input.limit
    });
    let cache_key = super::cache::key("getAssetsByAuthority", &params);
