use super::{AUCTIONEER_PROGRAM_ID, DISC_WITHDRAW, build_auctioneer_instruction, pda};
...
const NAME: &str = "auctioneer_withdraw";
const DEFINITION: &str = flow_lib::node_definition!("auctioneer/withdraw.jsonc");
...
pub struct Input {
    ...
    /// Escrow owner. Signs to authorize the withdraw.
    pub wallet: Wallet,
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let ix = build_auctioneer_instruction(DISC_WITHDRAW, accounts, args);
...
mod tests {
    ...
    fn test_instruction_construction() {
        let ix = build_auctioneer_instruction(DISC_WITHDRAW, vec![], vec![0u8; 10]);
