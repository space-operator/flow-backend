...
use super::{
    GovernanceInstruction, SPL_GOVERNANCE_ID, get_proposal_transaction_buffer_address,
    get_proposal_versioned_transaction_address,
};

const NAME: &str = "insert_versioned_transaction_from_buffer";

flow_lib::submit!(CommandDescription::new(NAME, |_| build()));

fn build() -> BuildResult {
    const DEFINITION: &str =
        flow_lib::node_definition!("/governance/insert_versioned_transaction_from_buffer.jsonc");
...
/// 5. `[writable]` ProposalTransactionBuffer account (PDA)
...
pub fn insert_versioned_transaction_from_buffer(
    ...
    let data = GovernanceInstruction::InsertVersionedTransactionFromBuffer {
        option_index,
        ephemeral_signers,
        transaction_index,
    };
    ...
    (
        ...
        proposal_transaction_buffer_address,
    )
}
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let (ix, proposal_versioned_tx_address, proposal_transaction_buffer_address) =
...
mod tests {
    ...
    fn test_instruction_builder() {
        ...
        let (ix, _, _) = insert_versioned_transaction_from_buffer(
            &SPL_GOVERNANCE_ID,
            &governance,
            &proposal,
            &token_owner_record,
