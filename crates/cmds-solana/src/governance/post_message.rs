...
fn build() -> BuildResult {
    const DEFINITION: &str = flow_lib::node_definition!("/governance/post_message.jsonc");
...
#[cfg(test)]
mod tests {
    use super::super::{MessageBody, SPL_GOVERNANCE_CHAT_ID};
    use super::*;
    ...
    fn test_instruction_builder() {
        ...
        let chat_message = Pubkey::new_unique();
        let payer = Pubkey::new_unique();

        let ix = post_message(
            &SPL_GOVERNANCE_CHAT_ID,
            &governance_program_id,
            &realm,
            &governance,
            &proposal,
            ...
            &payer,
            None,
            MessageBody::Text("test message".to_string()),
        );

        assert_eq!(ix.program_id, SPL_GOVERNANCE_CHAT_ID);
        assert!(!ix.data.is_empty());
        assert!(ix.accounts.len() >= 9);
    }
}
