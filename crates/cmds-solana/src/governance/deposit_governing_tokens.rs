...
fn build() -> BuildResult {
    const DEFINITION: &str =
        flow_lib::node_definition!("/governance/deposit_governing_tokens.jsonc");
...
/// 2. `[writable]` Governing token source
/// 3. `[signer]` Governing token owner
/// 4. `[signer]` Governing token source authority
/// 5. `[writable]` TokenOwnerRecord (PDA)
...
mod tests {
    ...
    fn test_instruction_builder() {
        ...
        let governing_token_source = Pubkey::new_unique();
        let governing_token_owner = Pubkey::new_unique();
        let governing_token_source_authority = Pubkey::new_unique();
        let payer = Pubkey::new_unique();
        let governing_token_mint = Pubkey::new_unique();

        let (ix, _, _, _) = deposit_governing_tokens(
            &SPL_GOVERNANCE_ID,
            &realm,
            &governing_token_source,
            &governing_token_owner,
            &governing_token_source_authority,
            &payer,
            1000u64,
