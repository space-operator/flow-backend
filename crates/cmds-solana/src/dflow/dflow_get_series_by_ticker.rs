...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let url = format!(
        "https://dev-prediction-markets-api.dflow.net/api/v1/series/{}",
        input.series_ticker
    );
...
mod tests {
    ...
    fn test_deserialize_response() {
        ...
        let _parsed: crate::dflow::response_types::Series =
            serde_json::from_str(json_str).expect("Failed to deserialize series.json");
