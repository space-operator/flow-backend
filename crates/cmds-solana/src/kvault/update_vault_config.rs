...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    data.extend(serde_json::to_vec(&input.entry)?);
    data.extend(borsh::to_vec(&input.data)?);
...
mod tests {
    ...
    /// Required fields: fee_payer, global_config, vault_state, klend_program, entry, data
    #[tokio::test]
