...
const NAME: &str = "claim_v2";
const DEFINITION: &str = flow_lib::node_definition!("jup_lock/claim_v2.jsonc");
...
mod tests {
    ...
    fn test_instruction_construction() {
        ...
        // claim_v2 has 9 accounts (claim has 7, v2 adds mint + memo_program)
    ...
    #[test]
    fn test_v2_includes_memo_program() {
        // Verify the memo program is in the account list (key difference from v1)
        let escrow = Pubkey::new_unique();
        let mint = Pubkey::new_unique();
        let recipient = Pubkey::new_unique();
        let token_program = solana_program::pubkey!("TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
        ...
        assert!(
            accounts
                .iter()
                .any(|a| a.pubkey == spl_memo_interface::v3::ID),
            "claim_v2 must include spl_memo_interface::v3::ID in accounts"
        );
    }
}
