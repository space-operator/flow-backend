...
pub const NAME: &str = "reflect_get_integration_events";
const DEFINITION: &str = flow_lib::node_definition!("reflect/get_integration_events.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = format!("/integration/{}/events", input.integration_id);
    let mut req = reflect_get(&ctx, &path);
    let mut query: Vec<(&str, &str)> = Vec::new();
    if let Some(ref val) = input.limit {
        query.push(("limit", val.as_str()));
    }
    if let Some(ref val) = input.period {
        query.push(("period", val.as_str()));
    }
    if let Some(ref val) = input.signer {
        query.push(("signer", val.as_str()));
    }
