...
use solana_attestation_service::instructions::ChangeSchemaStatusBuilder;

const NAME: &str = "change_schema_status";
const DEFINITION: &str =
    flow_lib::node_definition!("attestation_service/change_schema_status.jsonc");
...
pub async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let instruction = ChangeSchemaStatusBuilder::new()
...
mod tests {
    ...
    /// Required fields: fee_payer, authority, credential, schema, is_paused
    #[tokio::test]
    ...
    /// Integration test for change_schema_status using the run() function.
    /// Pauses or unpauses an existing schema.
    /// Returns a transaction signature.
    #[tokio::test]
    #[ignore = "requires funded wallet and network access"]
    async fn test_change_schema_status_integration() {
        ...
        // Step 3: Change schema status to paused using run()
        ...
        match result {
            Ok(output) => {
                ...
                println!(
                    "change_schema_status succeeded: {:?}",
                    output.signature.unwrap()
                );
            }
            Err(e) => {
                panic!("change_schema_status failed: {:?}", e);
            }
        }
    }
}
