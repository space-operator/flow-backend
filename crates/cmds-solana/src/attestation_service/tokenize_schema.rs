...
use solana_attestation_service::instructions::TokenizeSchemaBuilder;

const NAME: &str = "tokenize_schema";
const DEFINITION: &str = flow_lib::node_definition!("attestation_service/tokenize_schema.jsonc");
...
pub async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    // Derive schema_mint PDA from schema
    let (schema_mint, _bump) = pda::find_schema_mint(&input.schema);
    ...
    let instruction = TokenizeSchemaBuilder::new()
        ...
        .mint(to_pubkey_v2(&schema_mint))
        .max_size(super::calculate_schema_mint_size())
        .instruction();
    ...
    Ok(Output { signature, schema_mint })
}
...
mod tests {
    ...
    /// Required fields: fee_payer, payer, authority, credential, schema
    #[tokio::test]
    ...
    /// Integration test for tokenize_schema using the run() function.
    /// Tokenizes an existing schema to enable NFT attestations.
    /// Returns a transaction signature and the derived schema_mint PDA.
    #[tokio::test]
    #[ignore = "requires funded wallet and network access"]
    async fn test_tokenize_schema_integration() {
        ...
        let schema_input = create_schema::Input {
            ...
            description: "Test schema for tokenization".to_string(),
        ...
        // Expected schema_mint PDA
        let (expected_schema_mint, _) = pda::find_schema_mint(&schema);
        ...
        match result {
            Ok(output) => {
                ...
                assert_eq!(
                    output.schema_mint, expected_schema_mint,
                    "Schema mint PDA mismatch"
                );
                println!("tokenize_schema succeeded: {:?}", output.signature.unwrap());
            }
            Err(e) => {
                panic!("tokenize_schema failed: {:?}", e);
            }
        }
    }
}
