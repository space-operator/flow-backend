...
use super::{parse_strings, to_bytes, to_instruction_v3, to_pubkey_v2};
use solana_attestation_service::instructions::ChangeSchemaVersionBuilder;

const NAME: &str = "change_schema_version";
const DEFINITION: &str =
    flow_lib::node_definition!("attestation_service/change_schema_version.jsonc");
...
pub async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let instruction = ChangeSchemaVersionBuilder::new()
        ...
        .existing_schema(to_pubkey_v2(&input.existing_schema))
        .new_schema(to_pubkey_v2(&input.new_schema))
        .layout(to_bytes(&input.layout))
...
mod tests {
    ...
    /// Required fields: fee_payer, payer, authority, credential, existing_schema, new_schema, layout, field_names
    #[tokio::test]
    async fn test_input_parsing() {
        let input = value::map! {
            ...
            "existing_schema" => "GQZRKDqVzM4DXGGMEUNdnBD3CC4TTywh3PwgjYPBm8W9",
            "new_schema" => "GQZRKDqVzM4DXGGMEUNdnBD3CC4TTywh3PwgjYPBm8W9",
    ...
    /// Integration test for change_schema_version using the run() function.
    /// Creates a new version (v2) of an existing schema (v1).
    /// Returns a transaction signature.
    #[tokio::test]
    #[ignore = "requires funded wallet and network access"]
    async fn test_change_schema_version_integration() {
        ...
        // Step 2: Create schema v1 using run()
        let schema_input = create_schema::Input {
            ...
            description: "Schema v1".to_string(),
            layout: serde_json::json!([]),
            field_names: serde_json::json!([]),
            version: 1,
            submit: true,
        ...
        let existing_schema = schema_result.unwrap().schema;

        // Step 3: Create schema v2 (new version) using run()
        // Derive new_schema PDA for v2
        let (new_schema, _) = pda::find_schema(&credential, &schema_name, 2);
        ...
        match result {
            Ok(output) => {
                ...
                println!(
                    "change_schema_version succeeded: {:?}",
                    output.signature.unwrap()
                );
            }
            Err(e) => {
                panic!("change_schema_version failed: {:?}", e);
            }
        }
    }
}
