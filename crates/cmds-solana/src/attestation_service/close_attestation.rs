...
use solana_attestation_service::instructions::CloseAttestationBuilder;

const NAME: &str = "close_attestation";
const DEFINITION: &str = flow_lib::node_definition!("attestation_service/close_attestation.jsonc");
...
mod tests {
    ...
    use super::*;
    // Re-import to shadow the prelude's Input type
    use super::Input;
    ...
    /// Required fields: fee_payer, payer, authority, credential, attestation
    #[tokio::test]
    ...
    /// Integration test for close_attestation using the run() function.
    /// Closes (revokes) an existing attestation.
    /// Returns a transaction signature and the derived event_authority PDA.
    #[tokio::test]
    #[ignore = "requires funded wallet and network access"]
    async fn test_close_attestation_integration() {
        use crate::attestation_service::{create_attestation, create_credential, create_schema};
        ...
        let attest_input = create_attestation::Input {
            fee_payer: wallet.clone(),
            payer: wallet.clone(),
            authority: wallet.clone(),
            credential,
        ...
        let attest_result = create_attestation::run(CommandContext::default(), attest_input).await;
        assert!(
            attest_result.is_ok(),
            "create_attestation failed: {:?}",
            attest_result.err()
        );
        let attestation = attest_result.unwrap().attestation;

        // Step 4: Close attestation using run()
        ...
        match result {
            Ok(output) => {
                ...
                println!(
                    "close_attestation succeeded: {:?}",
                    output.signature.unwrap()
                );
            }
            Err(e) => {
                panic!("close_attestation failed: {:?}", e);
            }
        }
    }
}
