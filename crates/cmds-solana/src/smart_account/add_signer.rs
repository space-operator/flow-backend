...
const NAME: &str = "smart_account_add_signer";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/add_signer.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // AddSignerArgs { new_signer: SmartAccountSigner { key, permissions }, memo }
    let mut args_data = Vec::new();
    // SmartAccountSigner
    args_data.extend_from_slice(input.new_signer.as_ref());
    args_data.push(input.permissions);
    // memo: Option<String>
    match &input.memo {
        Some(s) => {
            args_data.push(1);
    ...
    let instruction = build_instruction("add_signer_as_authority", accounts, args_data);
