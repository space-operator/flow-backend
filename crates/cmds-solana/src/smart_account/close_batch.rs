...
const NAME: &str = "smart_account_close_batch";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/close_batch.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let accounts = vec![
        ...
        AccountMeta::new(input.batch_rent_collector, false),
        AccountMeta::new_readonly(solana_system_interface::program::ID, false),
    ];

    let instruction = build_instruction("close_batch", accounts, vec![]);
