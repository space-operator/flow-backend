...
const NAME: &str = "smart_account_remove_signer";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/remove_signer.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    args_data.extend_from_slice(input.old_signer.as_ref());
    match &input.memo {
        Some(s) => {
            args_data.push(1);
    ...
    let instruction = build_instruction("remove_signer_as_authority", accounts, args_data);
