...
const NAME: &str = "smart_account_set_settings_authority";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/set_settings_authority.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    args_data.extend_from_slice(input.new_settings_authority.as_ref());
    match &input.memo {
        Some(s) => {
            args_data.push(1);
    ...
    let instruction = build_instruction(
        "set_new_settings_authority_as_authority",
        accounts,
        args_data,
    );
