...
const NAME: &str = "smart_account_set_archival_authority";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/set_archival_authority.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    match input.new_archival_authority {
        Some(pk) => {
            args_data.push(1);
    ...
    let instruction = build_instruction("set_archival_authority_as_authority", accounts, args_data);
