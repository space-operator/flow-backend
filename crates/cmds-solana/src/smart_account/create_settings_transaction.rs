...
const NAME: &str = "smart_account_create_settings_transaction";
const DEFINITION: &str =
    flow_lib::node_definition!("smart_account/create_settings_transaction.jsonc");
...
pub struct Input {
    ...
    /// Borsh-serialized SettingsAction vec
    pub actions: Vec<u8>,
    #[serde(default)]
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // actions is pre-serialized by the caller
    let mut args_data = Vec::new();
    args_data.extend_from_slice(&input.actions);
    match &input.memo {
        Some(s) => {
            args_data.push(1);
    ...
    let instruction = build_instruction("create_settings_transaction", accounts, args_data);
