...
const NAME: &str = "smart_account_change_threshold";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/change_threshold.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    args_data.extend_from_slice(&input.new_threshold.to_le_bytes());
    match &input.memo {
        Some(s) => {
            args_data.push(1);
    ...
    let instruction = build_instruction("change_threshold_as_authority", accounts, args_data);
