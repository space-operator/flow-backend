...
const NAME: &str = "smart_account_create_batch";
const DEFINITION: &str = flow_lib::node_definition!("smart_account/create_batch.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let (batch, _) = pda::find_transaction(&input.settings, input.transaction_index);
    ...
    // CreateBatchArgs { account_index: u8, memo: Option<String> }
    ...
    let instruction = build_instruction("create_batch", accounts, args_data);
