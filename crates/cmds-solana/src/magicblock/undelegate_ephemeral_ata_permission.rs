use super::{
    ETOKEN_PROGRAM_ID, MAGIC_CONTEXT_ID, MAGIC_PROGRAM_ID, PERMISSION_PROGRAM_ID, discriminators,
    pda,
};
use crate::prelude::*;
use solana_program::instruction::{AccountMeta, Instruction};

const NAME: &str = "undelegate_ephemeral_ata_permission";
const DEFINITION: &str =
    flow_lib::node_definition!("magicblock/undelegate_ephemeral_ata_permission.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let data = discriminators::UNDELEGATE_EPHEMERAL_ATA_PERMISSION.to_vec();
