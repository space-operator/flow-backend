...
const NAME: &str = "delegate_ephemeral_ata";
const DEFINITION: &str = flow_lib::node_definition!("magicblock/delegate_ephemeral_ata.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let buffer = pda::delegation_buffer(&ephemeral_ata, &ETOKEN_PROGRAM_ID);
    let delegation_record = pda::delegation_record(&ephemeral_ata);
    let delegation_metadata = pda::delegation_metadata(&ephemeral_ata);
    ...
    let data = discriminators::DELEGATE_EPHEMERAL_ATA.to_vec();
