...
const NAME: &str = "set_immutable";
const DEFINITION: &str = flow_lib::node_definition!("escrow/set_immutable.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let instruction = build_escrow_instruction(EscrowDiscriminator::SetImmutable, accounts, vec![]);
...
mod tests {
    ...
    fn test_instruction_construction() {
        ...
        let ix = build_escrow_instruction(EscrowDiscriminator::SetImmutable, accounts, vec![]);

        assert_eq!(ix.program_id, ESCROW_PROGRAM_ID);
        assert_eq!(ix.accounts.len(), 4);
        assert_eq!(ix.data.len(), 1); // discriminator only
        assert_eq!(ix.data[0], 12); // SetImmutable discriminator
    }
}
