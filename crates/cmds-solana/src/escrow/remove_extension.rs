...
const NAME: &str = "remove_extension";
const DEFINITION: &str = flow_lib::node_definition!("escrow/remove_extension.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // Data: extensions_bump (u8) + extension_type (u16 LE)
    let mut args_data = Vec::with_capacity(3);
    args_data.push(bump);
    args_data.extend_from_slice(&input.extension_type.to_le_bytes());

    let instruction =
        build_escrow_instruction(EscrowDiscriminator::RemoveExtension, accounts, args_data);
...
mod tests {
    ...
    fn test_instruction_construction() {
        ...
        let mut args_data = Vec::with_capacity(3);
        args_data.push(bump);
        args_data.extend_from_slice(&2u16.to_le_bytes());
        ...
        assert_eq!(ix.data.len(), 4); // 1 discriminator + 1 bump + 2 extension_type
        assert_eq!(ix.data[0], 10); // RemoveExtension discriminator
    }
}
