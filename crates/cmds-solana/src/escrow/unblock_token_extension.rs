...
const NAME: &str = "unblock_token_extension";
const DEFINITION: &str = flow_lib::node_definition!("escrow/unblock_token_extension.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // Data: extensions_bump (u8) + blocked_extension (u16 LE)
    ...
    let instruction = build_escrow_instruction(
        EscrowDiscriminator::UnblockTokenExtension,
        accounts,
        args_data,
    );
...
mod tests {
    ...
    fn test_instruction_construction() {
        ...
        assert_eq!(ix.data[0], 11); // UnblockTokenExtension discriminator
    }
}
