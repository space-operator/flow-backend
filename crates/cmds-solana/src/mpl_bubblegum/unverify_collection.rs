...
use crate::prelude::*;
use mpl_bubblegum::instructions::UnverifyCollectionBuilder;
use solana_program::instruction::AccountMeta;
use solana_program::pubkey::Pubkey;
use std::str::FromStr;

const NAME: &str = "unverify_collection_cNFT";

const DEFINITION: &str = flow_lib::node_definition!("mpl_bubblegum/unverify_collection.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let ix = UnverifyCollectionBuilder::new()
        .tree_config(tree_config)
        .leaf_owner(input.leaf_owner)
        .leaf_delegate(leaf_delegate)
        .merkle_tree(merkle_tree)
        .payer(input.payer.pubkey())
        .tree_creator_or_delegate(input.tree_creator_or_delegate)
    ...
    let ins = Instructions {
        ...
        signers: [input.payer, input.collection_authority].into(),
