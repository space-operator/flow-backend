use super::{
    GetAssetResponse, MetadataBubblegum, WalletOrPubkey,
    types::asset::{Asset, AssetProof},
};
use crate::prelude::*;
use mpl_bubblegum::instructions::SetAndVerifyCollectionBuilder;
use solana_program::instruction::AccountMeta;
use solana_program::pubkey::Pubkey;
use std::str::FromStr;

const NAME: &str = "set_and_verify_collection_cNFT";

const DEFINITION: &str =
    flow_lib::node_definition!("mpl_bubblegum/set_and_verify_collection.jsonc");
...
pub struct Input {
    ...
    pub tree_creator_or_delegate: WalletOrPubkey,
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // Resolve tree_creator_or_delegate: can be signer (Wallet) or non-signer (Pubkey)
    let (tcd_pubkey, tcd_is_signer) = match &input.tree_creator_or_delegate {
        WalletOrPubkey::Wallet(w) => (w.pubkey(), true),
        WalletOrPubkey::Pubkey(p) => (*p, false),
    };
    ...
    let ix = SetAndVerifyCollectionBuilder::new()
        ...
        .tree_creator_or_delegate(tcd_pubkey, tcd_is_signer)
    ...
    // Build signers list: payer + collection_authority + optionally tree_creator_or_delegate
    let mut signers: Vec<Wallet> = vec![input.payer, input.collection_authority];
    if let WalletOrPubkey::Wallet(w) = input.tree_creator_or_delegate {
        signers.push(w);
    }
