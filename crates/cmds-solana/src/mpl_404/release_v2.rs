...
const NAME: &str = "release_v2";

flow_lib::submit!(CommandDescription::new(NAME, |_| build()));

fn build() -> BuildResult {
    const DEFINITION: &str = flow_lib::node_definition!("mpl_404/release_v2.json");
...
async fn run(mut ctx: Context, input: Input) -> Result<Output, CommandError> {
    ...
    let ix = Instructions {
        ...
        instructions: [release_v2_ix].into(),
    };
...
mod tests {
    ...
    async fn setup(
        ...
        let init_ata_if_needed_signature =
            init_ata_if_needed(ctx, payer.clone_keypair(), payer.pubkey(), token)
                .await
                .unwrap();

        dbg!(init_ata_if_needed_signature);

        let capture_v2_signature = capture_v2(
            ctx,
            CaptureV2Accounts {
                owner: payer.clone_keypair(),
                authority: authority.clone_keypair(),
                asset: asset.clone_keypair().pubkey(),
                collection: collection.clone_keypair().pubkey(),
                token,
