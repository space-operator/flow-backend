...
pub const NAME: &str = "bridge_list_customer_external_accounts";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_customer_external_accounts.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = format!("/v0/customers/{}/external_accounts", input.customer_id);
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_live_list_customer_external_accounts() {
        ...
        let resp = client
            .get(format!("https://api.sandbox.bridge.xyz/v0/customers/{customer_id}/external_accounts"))
            .header("Api-Key", &api_key)
            .query(&[("limit", "2")])
