...
pub const NAME: &str = "bridge_get_wallet_history";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_get_wallet_history.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = format!("/v0/wallets/{}/history", input.wallet_id);
