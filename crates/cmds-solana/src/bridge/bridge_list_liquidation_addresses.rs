...
pub const NAME: &str = "bridge_list_liquidation_addresses";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_liquidation_addresses.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = "/v0/liquidation_addresses";
...
mod tests {
    ...
    fn test_deserialize_response() {
        let json = std::fs::read_to_string(
            format!("{}/tests/fixtures/liquidation_address_list.json", env!("CARGO_MANIFEST_DIR"))
        ).unwrap();
        let _parsed: crate::bridge::response_types::PaginatedList<crate::bridge::response_types::LiquidationAddress> = serde_json::from_str(&json).unwrap();
    }
    ...
    async fn test_live_list_liquidation_addresses() {
        ...
        let resp = client
            .get("https://api.sandbox.bridge.xyz/v0/liquidation_addresses")
