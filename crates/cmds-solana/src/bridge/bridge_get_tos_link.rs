...
pub const NAME: &str = "bridge_get_tos_link";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_get_tos_link.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = format!("/v0/customers/{}/tos_acceptance_link", input.customer_id);
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_live_get_tos_link() {
        ...
        let resp = client
            .get(format!("https://api.sandbox.bridge.xyz/v0/customers/{customer_id}/tos_acceptance_link"))
            .header("Api-Key", &api_key)
