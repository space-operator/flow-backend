...
pub const NAME: &str = "bridge_list_virtual_accounts";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_virtual_accounts.jsonc");
...
mod tests {
    ...
    fn test_deserialize_response() {
        let json = std::fs::read_to_string(
            format!("{}/tests/fixtures/virtual_account_list.json", env!("CARGO_MANIFEST_DIR"))
        ).unwrap();
        let _parsed: crate::bridge::response_types::PaginatedList<crate::bridge::response_types::VirtualAccount> = serde_json::from_str(&json).unwrap();
    }

    #[tokio::test]
    #[ignore]
    async fn test_live_list_virtual_accounts() {
        ...
        let resp = client
            .get("https://api.sandbox.bridge.xyz/v0/virtual_accounts")
