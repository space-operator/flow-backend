...
pub const NAME: &str = "bridge_list_transfers";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_transfers.jsonc");
...
mod tests {
    ...
    fn test_deserialize_response() {
        let json = std::fs::read_to_string(
            format!("{}/tests/fixtures/transfer_list.json", env!("CARGO_MANIFEST_DIR"))
        ).unwrap();
        let _parsed: crate::bridge::response_types::PaginatedList<crate::bridge::response_types::Transfer> = serde_json::from_str(&json).unwrap();
    }
    ...
    async fn test_live_list_transfers() {
        ...
        let resp = client
            .get("https://api.sandbox.bridge.xyz/v0/transfers")
