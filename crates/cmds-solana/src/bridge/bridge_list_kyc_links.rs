...
pub const NAME: &str = "bridge_list_kyc_links";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_kyc_links.jsonc");
...
mod tests {
    ...
    fn test_deserialize_response() {
        let json = std::fs::read_to_string(
            format!("{}/tests/fixtures/kyc_link_list.json", env!("CARGO_MANIFEST_DIR"))
        ).unwrap();
        let _parsed: crate::bridge::response_types::PaginatedList<crate::bridge::response_types::KycLink> = serde_json::from_str(&json).unwrap();
    }
    ...
    async fn test_live_list_kyc_links() {
        ...
        let resp = client
            .get("https://api.sandbox.bridge.xyz/v0/kyc_links")
