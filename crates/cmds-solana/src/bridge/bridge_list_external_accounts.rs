...
pub const NAME: &str = "bridge_list_external_accounts";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_external_accounts.jsonc");
...
mod tests {
    ...
    fn test_deserialize_response() {
        let json = std::fs::read_to_string(
            format!("{}/tests/fixtures/external_account_list.json", env!("CARGO_MANIFEST_DIR"))
        ).unwrap();
        let _parsed: crate::bridge::response_types::PaginatedList<crate::bridge::response_types::ExternalAccount> = serde_json::from_str(&json).unwrap();
    }
    ...
    async fn test_live_list_external_accounts() {
        ...
        let resp = client
            .get("https://api.sandbox.bridge.xyz/v0/external_accounts")
