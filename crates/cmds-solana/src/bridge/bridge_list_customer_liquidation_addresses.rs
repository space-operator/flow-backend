...
pub const NAME: &str = "bridge_list_customer_liquidation_addresses";
const DEFINITION: &str = flow_lib::node_definition!("bridge/bridge_list_customer_liquidation_addresses.jsonc");
...
async fn run(ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    let path = format!("/v0/customers/{}/liquidation_addresses", input.customer_id);
...
mod tests {
    ...
    #[tokio::test]
    #[ignore]
    async fn test_live_list_customer_liquidation_addresses() {
        ...
        let resp = client
            .get(format!("https://api.sandbox.bridge.xyz/v0/customers/{customer_id}/liquidation_addresses"))
            .header("Api-Key", &api_key)
            .query(&[("limit", "2")])
