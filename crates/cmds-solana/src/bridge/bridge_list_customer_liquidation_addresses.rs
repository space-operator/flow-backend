...
const DEFINITION: &str =
    flow_lib::node_definition!("bridge/bridge_list_customer_liquidation_addresses.jsonc");
...
mod tests {
    ...
    async fn test_live_list_customer_liquidation_addresses() {
        ...
        let resp = client
            .get(format!(
                "https://api.sandbox.bridge.xyz/v0/customers/{customer_id}/liquidation_addresses"
            ))
            .header("Api-Key", &api_key)
            .query(&[("limit", "2")])
            .send()
