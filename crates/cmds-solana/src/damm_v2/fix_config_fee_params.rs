...
const NAME: &str = "fix_config_fee_params";
const DEFINITION: &str = flow_lib::node_definition!("damm_v2/fix_config_fee_params.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let accounts = vec![
        AccountMeta::new(input.config, false),                     // config (writable)
    ...
    data.extend(borsh::to_vec(&input.base_fee_data)?);
...
mod tests {
    ...
    /// Required fields: fee_payer, config, operator, signer, base_fee_data
    #[tokio::test]
    async fn test_input_parsing() {
        let input = value::map! {
            ...
            "base_fee_data" => "test_base_fee_data",
            "submit" => false,
        };
    ...
    async fn test_run() {
        ...
        let input = Input {
            ...
            base_fee_data: "base_fee_data".to_string(),
            submit: false,
        };
