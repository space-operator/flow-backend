use super::{
    ATA_PROGRAM_ID, CP_AMM_PROGRAM_ID, CUSTOMIZABLE_POOL_PREFIX, POSITION_NFT_ACCOUNT_PREFIX,
    SYSTEM_PROGRAM_ID, TOKEN_PROGRAM_ID, anchor_discriminator, derive_event_authority,
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let pool = Pubkey::find_program_address(
        &[
            ...
            input.token_a_mint.as_ref(),
            input.token_b_mint.as_ref(),
            input.creator.as_ref(),
        ],
        &CP_AMM_PROGRAM_ID,
    )
    .0;
