...
const NAME: &str = "migration_damm_v2_create_metadata";
const DEFINITION: &str = flow_lib::node_definition!("dynamic_bonding_curve/migration_damm_v2_create_metadata.jsonc");
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let accounts = vec![
        ...
        AccountMeta::new_readonly(migration_metadata, false),    // 2: migration_metadata (readonly)
        AccountMeta::new_readonly(input.payer, false),           // 3: payer (readonly)
    ...
    let data = discriminators::MIGRATION_DAMM_V2_CREATE_METADATA.to_vec();
