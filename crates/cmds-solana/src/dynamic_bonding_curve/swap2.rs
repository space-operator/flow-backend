...
pub struct Input {
    ...
    /// Amount in (flexible based on swap_type)
    pub amount_in: u64,
    /// Amount out (flexible based on swap_type)
    pub amount_out: u64,
    /// Swap type: 0 = ExactIn, 1 = ExactOut
    pub swap_type: u8,
...
async fn run(mut ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    // Swap2Parameters { amount_in: u64, amount_out: u64, swap_type: SwapType }
    let mut data = discriminators::SWAP2.to_vec();
    data.extend(borsh::to_vec(&input.amount_in)?);
    data.extend(borsh::to_vec(&input.amount_out)?);
    data.extend(borsh::to_vec(&input.swap_type)?);
