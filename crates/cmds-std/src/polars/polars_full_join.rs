...
pub const NAME: &str = "polars_full_join";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_full_join.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let mut result = if let Some(ref on) = input.on {
        ...
        left.join(&right, on_cols.clone(), on_cols, JoinArgs::new(JoinType::Full), None)
            .map_err(|e| CommandError::msg(format!("Full join error: {e}")))?
    } else {
