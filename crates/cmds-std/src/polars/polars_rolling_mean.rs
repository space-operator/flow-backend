...
pub const NAME: &str = "polars_rolling_mean";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_rolling_mean.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let output_col = format!("{}_rolling_mean", input.column);
    ...
    let mut result = df
        .lazy()
        .with_column(col(&input.column).rolling_mean(opts).alias(&output_col))
        .collect()
        .map_err(|e| CommandError::msg(format!("Rolling mean error: {e}")))?;
...
mod tests {
    ...
    fn test_df_ipc() -> String {
        let mut df = DataFrame::new(vec![
            Series::new("value".into(), &[10.0f64, 20.0, 30.0, 40.0, 50.0]).into_column(),
        ]).unwrap();
    ...
    async fn test_run() {
        ...
        assert_eq!(df.height(), 5, "rolling_mean should preserve row count");
        let rolling = df.column("value_rolling_mean").unwrap();
        // First 2 values are null (min_periods=3, not enough window)
        ...
        assert!((v3 - 30.0).abs() < 1e-10);
        // mean(30,40,50) = 40.0
        let v4: f64 = rolling.f64().unwrap().get(4).unwrap();
        assert!((v4 - 40.0).abs() < 1e-10);
    }
}
