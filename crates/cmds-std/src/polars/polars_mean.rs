...
pub const NAME: &str = "polars_mean";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_mean.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let exprs: Vec<Expr> = if let Some(ref cols) = input.columns {
        ...
        col_names.iter().map(|c| col(c.as_str()).mean()).collect()
    } else {
        df.get_column_names()
            .iter()
    ...
    let mut result = df
        ...
        .select(exprs)
        .collect()
        .map_err(|e| CommandError::msg(format!("Mean error: {e}")))?;
