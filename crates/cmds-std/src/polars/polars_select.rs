...
pub const NAME: &str = "polars_select";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_select.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let mut selected = df
        .select(cols)
        .map_err(|e| CommandError::msg(format!("Select error: {e}")))?;
    let (ipc, json) = dual_output(&mut selected)?;
    Ok(Output {
        dataframe: ipc,
        dataframe_json: json,
    })
...
mod tests {
    ...
    async fn test_run_select() {
        let output = run(CommandContext::default(), Input {
            ...
            columns: serde_json::json!(["name", "age"]),
        }).await.unwrap();
