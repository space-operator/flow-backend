...
pub const NAME: &str = "polars_max";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_max.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let exprs: Vec<Expr> = if let Some(ref cols) = input.columns {
        ...
        col_names.iter().map(|c| col(c.as_str()).max()).collect()
    } else {
        df.get_column_names()
            .iter()
...
mod tests {
    ...
    async fn test_run_max() {
        ...
        assert_eq!(val, AnyValue::Int64(50));
    }
}
