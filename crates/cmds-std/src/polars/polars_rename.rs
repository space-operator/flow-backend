...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let mapping = mapping_value.as_object().ok_or_else(|| {
        CommandError::msg("mapping must be a JSON object {\"old_name\": \"new_name\", ...}")
    })?;

    for (old_name, new_value) in mapping {
        let new_name = new_value.as_str().ok_or_else(|| {
            CommandError::msg(format!("Rename value for '{old_name}' must be a string"))
        })?;
