...
pub const NAME: &str = "polars_series_add";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_series_add.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let result = (&l + &r).map_err(|e| CommandError::msg(format!("Add error: {e}")))?;
    ...
    async fn test_run_add() {
        let output = run(
            ...
            Input {
                ...
                right: test_series_ipc("b", &[10, 20, 30]),
            },
        )
        .await
        .unwrap();
