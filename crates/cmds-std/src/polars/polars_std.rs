...
pub const NAME: &str = "polars_std";
const DEFINITION: &str = flow_lib::node_definition!("polars/polars_std.jsonc");
...
async fn run(_ctx: CommandContext, input: Input) -> Result<Output, CommandError> {
    ...
    let exprs: Vec<Expr> = if let Some(ref cols) = input.columns {
        ...
        col_names
            .iter()
            .map(|c| col(c.as_str()).std(input.ddof))
            .collect()
    } else {
        df.get_column_names()
            .iter()
            .map(|name| col(name.as_str()).std(input.ddof))
            .collect()
    };
    ...
    let mut result = df
        ...
        .select(exprs)
        .collect()
        .map_err(|e| CommandError::msg(format!("Std error: {e}")))?;
...
mod tests {
    ...
    async fn test_run_std() {
        ...
        assert_eq!(df.height(), 1, "std should produce a 1-row DataFrame");
        let val: f64 = df.column("value").unwrap().f64().unwrap().get(0).unwrap();
        // std with ddof=1 of [10,20,30,40,50] = sqrt(250) ~ 15.811
        assert!((val - 250.0f64.sqrt()).abs() < 0.01, "expected ~15.81, got {val}");
    }
}
